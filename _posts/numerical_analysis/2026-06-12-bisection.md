...
# 개념
이분법은 구간 $[a_0,b_0]$에서 연속인 함수 $f(x)$의 해를 찾는 방법으로, 중간값의 정리를 이용한다.
...
3. 다음과 같이 구간을 갱신한다.  
   - $f(a_0)f(m_0)<0$이면 $a_1=a_0,\ b_1=m_0$
   - $f(m_0)f(b_0)<0$이면 $a_1=m_0,\ b_1=b_0$
4. $\lvert b_n-a_n \rvert < \epsilon$ 또는 $\lvert f(m_n) \rvert < \epsilon$이 될 때까지 반복한다.

#  수렴성
함수 $f(x)$가 폐구간 $[a_0,b_0]$에서 연속이고 $f(a_0)f(b_0)<0$이면, 이분법에 의하여 만들어진 수열 $\{m_n\}$은 해 $p$에 수렴한다.
...
$$
b_n-a_n=\frac{1}{2}(b_{n-1}-a_{n-1})=\frac{1}{2^{n}}(b_0-a_0)
$$

따라서 실제 근을 $p$라고 하고 $m_n=\frac{a_n+b_n}{2}$라고 하면 다음의 부등식이 성립한다.

$$
|m_n-p|\le m_n-a_n=\frac{1}{2}(b_n-a_n)=\frac{1}{2^{n+1}}(b_0-a_0)\le \epsilon
$$
